...
function game(_works) {
    let play = true;
    let round = 0;
    let countRightAnswer = 0;
    while (true) {
        let question = generateText(_works[round]);
        ...
        round = getNextRound(_works[round], answer);
    }
    alert("Игра завершена.");
}

function generateText(work) {
    let text = '';
    for (let i = 0; i < work.answer.length; i++) {
        text = text + work.answer[i];
    }
    return work.text + text;
}

function getNextRound(work, answer) {
    return work.jump[answer - 1];
}
